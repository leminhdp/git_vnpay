...
$table{'SEARCH_FILM'} = [0,0,0];
$table{'GET_TICKET_TYPE'} = [0,0,0];
$table{'GET_SEAT'} = [0,0,0];
$table{'CREATE_ORDER'} = [0,0,0];
$table{'CONFIRM_TICKET'} = [0,0,0];
...
#2018-07-20 10:01:28, action:SEARCH_FILM, cinema:57, code:00, time_process:0
#2018-07-20 10:01:37, action:GET_TICKET_TYPE, sessioncode:963304, code:00, time_process:0
#2018-07-19 07:12:21, action:GET_SEAT, queryId:217180, code:00, time_process:1
#2018-07-19 08:52:26, action:CREATE_ORDER, queryId:217013, code:00, time_process:0
#2018-07-19 07:14:25, action:CONFIRM_TICKET, queryId:217320, code:00, time_process:7
$pattern_timeout = "action:(SEARCH_FILM|GET_TICKET_TYPE|GET_SEAT|CREATE_ORDER|CONFIRM_TICKET), (cinema|sessioncode|queryId):(ALL|\\d+), code:(99|08), time_process:(\\d+)";
$pattern_successed = "action:(SEARCH_FILM|GET_TICKET_TYPE|GET_SEAT|CREATE_ORDER|CONFIRM_TICKET), (cinema|sessioncode|queryId):(ALL|\\d+), code:(00), time_process:(\\d+)";
$pattern_failed = "action:(SEARCH_FILM|GET_TICKET_TYPE|GET_SEAT|CREATE_ORDER|CONFIRM_TICKET), (cinema|sessioncode|queryId):(ALL|\\d+), code:(01), time_process:(\\d+)";
...
### note the following is done for each file that is found and matches the name and date criteria
		...
		while (defined($line = $LOGS->readline) ) {
			...
			if ($dt_tzadjusted > $oldestDate) { # is the date bigger=>newer than the oldest date we want to look at?
				...
				if ($line =~ m/$pattern_timeout/){ # if the line contains the regex pattern
					$table{$1}[0]++;
					$table{'ID_TIMEOUT'} = $table{'ID_TIMEOUT'}.$1."-".$2.':'.$4.',';
					if ($debug) { print "Timeout: $1 -- $2 -- $3 -- $4 -- $5\n";}				
					}
				if ($line =~ m/$pattern_failed/){ # if the line contains the regex pattern
					$table{$1}[2]++;
					$table{'ID_FAILED'} = $table{'ID_FAILED'}.$1."-".$2.':'.$4.',';
					$table{'TIME_PROCESS'} += $5;
	...
	my $trans_num = $table{'SEARCH_FILM'}[1] + $table{'GET_TICKET_TYPE'}[1] + $table{'GET_SEAT'}[1] + $table{'CREATE_ORDER'}[1] + $table{'CONFIRM_TICKET'}[1] + $table{'GET_SEAT'}[2]+ $table{'SEARCH_FILM'}[2] + $table{'GET_TICKET_TYPE'}[2]+ $table{'CREATE_ORDER'}[2] + $table{'CONFIRM_TICKET'}[2];
	if ($trans_num > 0) {
		$avg_time_process = $table{'TIME_PROCESS'}/$trans_num;
	}
		
	my $txt_request = '';	
	if (($table{'SEARCH_FILM'}[0] + $table{'GET_TICKET_TYPE'}[0] + $table{'GET_SEAT'}[0] + $table{'CREATE_ORDER'}[0] + $table{'CONFIRM_TICKET'}[0]) > ($critical + 0)) {
		if ($table{'SEARCH_FILM'}[0] > 0) { $txt_request = $table{'SEARCH_FILM'}[0].' yeu cau Tim Ve ';}
		if ($table{'GET_TICKET_TYPE'}[0] > 0) { $txt_request = $table{'SEARCH_FILM'}[0].' yeu cau Tim Ve va '.$table{'GET_TICKET_TYPE'}[0].' Dat Ve ';}
		if ($table{'GET_SEAT'}[0] > 0) { $txt_request = $table{'SEARCH_FILM'}[0].' yeu cau Tim Ve, '.$table{'GET_TICKET_TYPE'}[0].' Dat Ve va '.$table{'GET_SEAT'}[0].' Dat Ghe Ngoi ';}
		if ($table{'CREATE_ORDER'}[0] > 0) { $txt_request = $table{'SEARCH_FILM'}[0].' yeu cau Tim Ve, '.$table{'GET_TICKET_TYPE'}[0].' Dat Ve, '.$table{'GET_SEAT'}[0].' Dat Ghe Ngoi va '.$table{'CREATE_ORDER'}[0].' Tao Don Hang ';}
		if ($table{'CONFIRM_TICKET'}[0] > 0) { $txt_request = $table{'SEARCH_FILM'}[0].' yeu cau Tim Ve, '.$table{'GET_TICKET_TYPE'}[0].' Dat Ve, '.$table{'GET_SEAT'}[0].' Dat Ghe Ngoi, '.$table{'CREATE_ORDER'}[0].' Tao Don Hang va '.$table{'CONFIRM_TICKET'}[0].' Xac Nhan Dat Ve ';}
			
		
		
		print "Co ".$txt_request." bi Timeout trong ".$interval." phut qua, ID_Timeout(".$table{'ID_TIMEOUT'}.")";
		
		print "|TimVe_Timeout=".$table{'SEARCH_FILM'}[0];
		print " TimVe_Successed=".$table{'SEARCH_FILM'}[1];
		print " TimVe_Failed=".($table{'SEARCH_FILM'}[2]+$table{'SEARCH_FILM'}[0]);
		print " DatVe_Timeout=".$table{'GET_TICKET_TYPE'}[0];
		print " DatVe_Successed=".$table{'GET_TICKET_TYPE'}[1];
		print " DatVe_Failed=".($table{'GET_TICKET_TYPE'}[2]+$table{'GET_TICKET_TYPE'}[0]);
		...
		print " TaoDon_Timeout=".$table{'CREATE_ORDER'}[0];
		print " TaoDon_Successed=".$table{'CREATE_ORDER'}[1];
		print " TaoDon_Failed=".($table{'CREATE_ORDER'}[2]+$table{'GET_SEAT'}[0]);
		print " XacNhan_Timeout=".$table{'CONFIRM_TICKET'}[0];
		print " XacNhan_Successed=".$table{'CONFIRM_TICKET'}[1];
		print " XacNhan_Failed=".($table{'CONFIRM_TICKET'}[2]+$table{'CONFIRM_TICKET'}[0]);
		printf " Avg_Time_Process=%.1fs",$avg_time_process;
		
		exit 2;
	}	
	if (($table{'SEARCH_FILM'}[2] + $table{'GET_TICKET_TYPE'}[2] + $table{'GET_SEAT'}[2] + $table{'CREATE_ORDER'}[2] + $table{'CONFIRM_TICKET'}[2]) > ($warning + 0)) {
			if ($table{'SEARCH_FILM'}[2] > 0) { $txt_request = $table{'SEARCH_FILM'}[2].' yeu cau Tim Ve ';}
			if ($table{'GET_TICKET_TYPE'}[2] > 0) { $txt_request = $table{'SEARCH_FILM'}[2].' yeu cau Tim Ve va '.$table{'GET_TICKET_TYPE'}[2].' Dat Ve ';}
			if ($table{'GET_SEAT'}[2] > 0) { $txt_request = $table{'SEARCH_FILM'}[2].' yeu cau Tim Ve, '.$table{'GET_TICKET_TYPE'}[2].' Dat Ve va '.$table{'GET_SEAT'}[2].' Dat Ghe Ngoi ';}
			if ($table{'CREATE_ORDER'}[2] > 0) { $txt_request = $table{'SEARCH_FILM'}[2].' yeu cau Tim Ve, '.$table{'GET_TICKET_TYPE'}[2].' Dat Ve, '.$table{'GET_SEAT'}[2].' Dat Ghe Ngoi va '.$table{'CREATE_ORDER'}[2].' Tao Don Hang ';}
			if ($table{'CONFIRM_TICKET'}[2] > 0) { $txt_request = $table{'SEARCH_FILM'}[2].' yeu cau Tim Ve, '.$table{'GET_TICKET_TYPE'}[2].' Dat Ve, '.$table{'GET_SEAT'}[2].' Dat Ghe Ngoi, '.$table{'CREATE_ORDER'}[2].' Tao Don Hang, '.$table{'CONFIRM_TICKET'}[2].' Xac Nhan Dat Ve ';}
	...
	print "Co ".($table{'SEARCH_FILM'}[1] + $table{'GET_TICKET_TYPE'}[1]+ $table{'GET_SEAT'}[1] + $table{'CREATE_ORDER'}[1] + $table{'CONFIRM_TICKET'}[1])."/".($table{'SEARCH_FILM'}[0] + $table{'GET_TICKET_TYPE'}[0]+ $table{'GET_SEAT'}[0] + $table{'CREATE_ORDER'}[0] + $table{'CONFIRM_TICKET'}[0]+$table{'SEARCH_FILM'}[2] + $table{'GET_TICKET_TYPE'}[2]+ $table{'GET_SEAT'}[2] + $table{'CREATE_ORDER'}[2] + $table{'CONFIRM_TICKET'}[2])." giao dich Successed/Failed trong ".$interval." phut qua";
	print "|TimVe_Timeout=".$table{'SEARCH_FILM'}[0];
		print " TimVe_Successed=".$table{'SEARCH_FILM'}[1];
		print " TimVe_Failed=".($table{'SEARCH_FILM'}[2]+$table{'SEARCH_FILM'}[0]);
		print " DatVe_Timeout=".$table{'GET_TICKET_TYPE'}[0];
