...
### note the following is done for each file that is found and matches the name and date criteria
	...
	if (($table{'SEARCH_FILM'}[0] + $table{'GET_TICKET_TYPE'}[0] + $table{'GET_SEAT'}[0] + $table{'CREATE_ORDER'}[0] + $table{'CONFIRM_TICKET'}[0]) > ($critical + 0)) {
		if ($table{'GET_TICKET_TYPE'}[0] > 0 && $table{'SEARCH_FILM'}[0] && $table{'GET_SEAT'}[0] && $table{'CREATE_ORDER'}[0] && $table{'CONFIRM_TICKET'}[0]  > 0){ $txt_request = $table{'SEARCH_FILM'}[0].' yeu cau Tim Ve va '.$table{'GET_TICKET_TYPE'}[0].' giao dich Dat Ve'.$table{'GET_SEAT'}[0].' Dat ghe ngoi '.$table{'CREATE_ORDER'}[0].' Tao don hang '.$table{'CONFIRM_TICKET'}[0].' Vac nhan ve mua ';}
		else {
			if ($table{'SEARCH_FILM'}[0] > 0){ $txt_request = $table{'SEARCH_FILM'}[0].' yeu cau Tim Ve';}
			if ($table{'GET_SEAT'}[0] > 0){ $txt_request = $table{'GET_SEAT'}[0].' Dat ghe ngoi';}
			if ($table{'CREATE_ORDER'}[0] > 0){ $txt_request = $table{'CREATE_ORDER'}[0].' tao don hang';}
			if ($table{'CONFIRM_TICKET'}[0] > 0){ $txt_request = $table{'CONFIRM_TICKET'}[0].' xac nhan ve mua';}
			elsif ($table{'GET_TICKET_TYPE'}[0] > 0){ $txt_request = $table{'GET_TICKET_TYPE'}[0].' giao dich Dat Ve';}
			}
	...
	if (($table{'SEARCH_FILM'}[2] + $table{'GET_TICKET_TYPE'}[2] + $table{'GET_SEAT'}[2] + $table{'CREATE_ORDER'}[2] + $table{'CONFIRM_TICKET'}[2]) > ($warning + 0)) {
		if ($table{'GET_TICKET_TYPE'}[2] > 0 && $table{'SEARCH_FILM'}[2] && $table{'GET_SEAT'}[2] && $table{'CREATE_ORDER'}[2] && $table{'CONFIRM_TICKET'}[2]  > 0){ $txt_request = $table{'SEARCH_FILM'}[2].' yeu cau Tim Ve va '.$table{'GET_TICKET_TYPE'}[2].' giao dich Dat Ve'.$table{'GET_SEAT'}[2].' Dat ghe ngoi '.$table{'CREATE_ORDER'}[2].' Tao don hang '.$table{'CONFIRM_TICKET'}[2].' Vac nhan ve mua ';}
		else {
			if ($table{'SEARCH_FILM'}[2] > 0){ $txt_request = $table{'SEARCH_FILM'}[2].' yeu cau Tim Ve';}
			if ($table{'GET_SEAT'}[2] > 0){ $txt_request = $table{'GET_SEAT'}[2].' Dat ghe ngoi';}
			if ($table{'CREATE_ORDER'}[2] > 0){ $txt_request = $table{'CREATE_ORDER'}[2].' tao don hang';}
			if ($table{'CONFIRM_TICKET'}[2] > 0){ $txt_request = $table{'CONFIRM_TICKET'}[2].' xac nhan ve mua';}
			elsif ($table{'GET_TICKET_TYPE'}[2] > 0){ $txt_request = $table{'GET_TICKET_TYPE'}[2].' giao dich Dat Ve';}
			}
