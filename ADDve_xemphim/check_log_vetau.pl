...
$table{'SEARCH_TRAIN'} = [0,0,0];
$table{'HOLD_SEAT'} = [0,0,0];
$table{'GET_SEAT'} = [0,0,0];
$table{'BOOK'} = [0,0,0];
$table{'ISSUE_TICKET'} = [0,0,0];
$table{'REFUND_TICKET'} = [0,0,0];
...
#2018-07-20 10:01:28, action:SEARCH_TRAIN, cinema:57, code:00, time_process:0
#2018-07-20 10:01:37, action:HOLD_SEAT, sessioncode:963304, code:00, time_process:0
...
#2018-07-19 07:14:25, action:ISSUE_TICKET, queryId:217320, code:00, time_process:7


# 2018-11-04 01:06:16, action:SEARCH_TRAIN, code:00, time_process:1
# 2018-11-04 01:06:26, action:GET_SEAT, code:00, time_process:1, request_id:22234
# 2018-11-04 01:07:56, action:HOLD_SEAT, code:00, time_process:1, Request_id:22234
# 2018-11-04 01:08:03, action:BOOK, code:00, time_process:1, Request_id:22232
# 2018-11-04 01:08:03, action: ISSUE_TICKET, code:00, time_process:1, Request_id:22232
# 2018-11-04 01:08:03, action: REFUND_TICKET, code:00, time_process:1, Request_id:22232

$pattern_timeout = "action:(SEARCH_TRAIN|GET_SEAT|HOLD_SEAT|BOOK|ISSUE_TICKET|REFUND_TICKET), code:(99|08), time_process:(\\d+), ([Rr]equest_id|Book_Id):(\\d+)";
$pattern_successed = "action:(SEARCH_TRAIN|GET_SEAT|HOLD_SEAT|BOOK|ISSUE_TICKET|REFUND_TICKET), code:(00), time_process:(\\d+), ([Rr]equest_id|Book_Id):(\\d+)";
$pattern_failed = "action:(SEARCH_TRAIN|GET_SEAT|HOLD_SEAT|BOOK|ISSUE_TICKET|REFUND_TICKET), code:(01), time_process:(\\d+), ([Rr]equest_id|Book_Id):(\\d+)";
...
### note the following is done for each file that is found and matches the name and date criteria
	...
	my $trans_num = $table{'SEARCH_TRAIN'}[1] + $table{'HOLD_SEAT'}[1] + $table{'GET_SEAT'}[1] + $table{'BOOK'}[1] + $table{'ISSUE_TICKET'}[1] + $table{'REFUND_TICKET'}[1] + $table{'GET_SEAT'}[2]+ $table{'SEARCH_TRAIN'}[2] + $table{'HOLD_SEAT'}[2]+ $table{'BOOK'}[2] + $table{'ISSUE_TICKET'}[2] + + $table{'REFUND_TICKET'}[2];
	if ($trans_num > 0) {
		$avg_time_process = $table{'TIME_PROCESS'}/$trans_num;
	}
		
	my $txt_request = '';	
	if (($table{'SEARCH_TRAIN'}[0] + $table{'HOLD_SEAT'}[0] + $table{'GET_SEAT'}[0] + $table{'BOOK'}[0] + $table{'ISSUE_TICKET'}[0] + $table{'REFUND_TICKET'}[0]) > ($critical + 0)) {
		if ($table{'SEARCH_TRAIN'}[0] > 0) { $txt_request = $table{'SEARCH_TRAIN'}[0].' yeu cau Tim Ve ';}
		if ($table{'HOLD_SEAT'}[0] > 0) { $txt_request = $table{'SEARCH_TRAIN'}[0].' yeu cau Tim Ve va '.$table{'HOLD_SEAT'}[0].' Dat Ve ';}
		if ($table{'GET_SEAT'}[0] > 0) { $txt_request = $table{'SEARCH_TRAIN'}[0].' yeu cau Tim Ve, '.$table{'HOLD_SEAT'}[0].' Dat Ve va '.$table{'GET_SEAT'}[0].' Dat Ghe Ngoi ';}
		if ($table{'BOOK'}[0] > 0) { $txt_request = $table{'SEARCH_TRAIN'}[0].' yeu cau Tim Ve, '.$table{'HOLD_SEAT'}[0].' Dat Ve, '.$table{'GET_SEAT'}[0].' Dat Ghe Ngoi va '.$table{'BOOK'}[0].' Tao Don Hang ';}
		if ($table{'ISSUE_TICKET'}[0] > 0) { $txt_request = $table{'SEARCH_TRAIN'}[0].' yeu cau Tim Ve, '.$table{'HOLD_SEAT'}[0].' Dat Ve, '.$table{'GET_SEAT'}[0].' Dat Ghe Ngoi, '.$table{'BOOK'}[0].' Tao Don Hang va '.$table{'ISSUE_TICKET'}[0].' Xac Nhan Dat Ve ';}
		if ($table{'REFUND_TICKET'}[0] > 0) { $txt_request = $table{'SEARCH_TRAIN'}[0].' yeu cau Tim Ve, '.$table{'HOLD_SEAT'}[0].' Dat Ve, '.$table{'GET_SEAT'}[0].' Dat Ghe Ngoi, '.$table{'BOOK'}[0].' Tao Don Hang va '.$table{'ISSUE_TICKET'}[0].' Xac Nhan Dat Ve, '.$table{'REFUND_TICKET'}[0].' Xac Nhan Hoan Tien ';}
			
		
		
		print "Co ".$txt_request." bi Timeout trong ".$interval." phut qua, ID_Timeout(".$table{'ID_TIMEOUT'}.")";
		
		print "|TimVe_Timeout=".$table{'SEARCH_TRAIN'}[0];
		print " TimVe_Successed=".$table{'SEARCH_TRAIN'}[1];
		print " TimVe_Failed=".($table{'SEARCH_TRAIN'}[2]+$table{'SEARCH_TRAIN'}[0]);
		print " DatVe_Timeout=".$table{'HOLD_SEAT'}[0];
		print " DatVe_Successed=".$table{'HOLD_SEAT'}[1];
		print " DatVe_Failed=".($table{'HOLD_SEAT'}[2]+$table{'HOLD_SEAT'}[0]);
		...
		print " XacNhan_Timeout=".$table{'ISSUE_TICKET'}[0];
		print " XacNhan_Successed=".$table{'ISSUE_TICKET'}[1];
		print " XacNhan_Failed=".($table{'ISSUE_TICKET'}[2]+$table{'ISSUE_TICKET'}[0]);
		print " HoanTien_Timeout=".$table{'REFUND_TICKET'}[0];
		print " HoanTien_Successed=".$table{'REFUND_TICKET'}[1];
		print " HoanTien_Failed=".($table{'REFUND_TICKET'}[2]+$table{'REFUND_TICKET'}[0]);
		printf " Avg_Time_Process=%.1fs",$avg_time_process;
		
		exit 2;
	}	
	if (($table{'SEARCH_TRAIN'}[2] + $table{'HOLD_SEAT'}[2] + $table{'GET_SEAT'}[2] + $table{'BOOK'}[2] + $table{'ISSUE_TICKET'}[2] + $table{'REFUND_TICKET'}[2]) > ($warning + 0)) {
			if ($table{'SEARCH_TRAIN'}[2] > 0) { $txt_request = $table{'SEARCH_TRAIN'}[2].' yeu cau Tim Ve ';}
			if ($table{'HOLD_SEAT'}[2] > 0) { $txt_request = $table{'SEARCH_TRAIN'}[2].' yeu cau Tim Ve va '.$table{'HOLD_SEAT'}[2].' Dat Ve ';}
			if ($table{'GET_SEAT'}[2] > 0) { $txt_request = $table{'SEARCH_TRAIN'}[2].' yeu cau Tim Ve, '.$table{'HOLD_SEAT'}[2].' Dat Ve va '.$table{'GET_SEAT'}[2].' Dat Ghe Ngoi ';}
			if ($table{'BOOK'}[2] > 0) { $txt_request = $table{'SEARCH_TRAIN'}[2].' yeu cau Tim Ve, '.$table{'HOLD_SEAT'}[2].' Dat Ve, '.$table{'GET_SEAT'}[2].' Dat Ghe Ngoi va '.$table{'BOOK'}[2].' Tao Don Hang ';}
			if ($table{'ISSUE_TICKET'}[2] > 0) { $txt_request = $table{'SEARCH_TRAIN'}[2].' yeu cau Tim Ve, '.$table{'HOLD_SEAT'}[2].' Dat Ve, '.$table{'GET_SEAT'}[2].' Dat Ghe Ngoi, '.$table{'BOOK'}[2].' Tao Don Hang, '.$table{'ISSUE_TICKET'}[2].' Xac Nhan Dat Ve ';}
			if ($table{'REFUND_TICKET'}[2] > 0) { $txt_request = $table{'SEARCH_TRAIN'}[2].' yeu cau Tim Ve, '.$table{'HOLD_SEAT'}[2].' Dat Ve, '.$table{'GET_SEAT'}[2].' Dat Ghe Ngoi, '.$table{'BOOK'}[2].' Tao Don Hang, '.$table{'ISSUE_TICKET'}[2].' Xac Nhan Dat Ve, '.$table{'REFUND_TICKET'}[2].' Xac Nhan Hoan Tien';}
	...
	print "Co ".($table{'SEARCH_TRAIN'}[1] + $table{'HOLD_SEAT'}[1]+ $table{'GET_SEAT'}[1] + $table{'BOOK'}[1] + $table{'ISSUE_TICKET'}[1] + $table{'REFUND_TICKET'}[1])."/".($table{'SEARCH_TRAIN'}[0] + $table{'HOLD_SEAT'}[0]+ $table{'GET_SEAT'}[0] + $table{'BOOK'}[0] + $table{'ISSUE_TICKET'}[0] + $table{'REFUND_TICKET'}[0] + $table{'SEARCH_TRAIN'}[2] + $table{'HOLD_SEAT'}[2]+ $table{'GET_SEAT'}[2] + $table{'BOOK'}[2] + $table{'ISSUE_TICKET'}[2] + $table{'REFUND_TICKET'}[2])." giao dich Successed/Failed trong ".$interval." phut qua";
	print "|TimVe_Timeout=".$table{'SEARCH_TRAIN'}[0];
		print " TimVe_Successed=".$table{'SEARCH_TRAIN'}[1];
		print " TimVe_Failed=".($table{'SEARCH_TRAIN'}[2]+$table{'SEARCH_TRAIN'}[0]);
		print " DatVe_Timeout=".$table{'HOLD_SEAT'}[0];
